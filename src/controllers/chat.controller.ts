...
chatRouter.get("/rooms", async (req: JWTAuthRequest, res: any) => {
  try {
    if (!req.user?.userId) {
...
chatRouter.post("/rooms", async (req: JWTAuthRequest, res: any) => {
  try {
    if (!req.user?.userId) {
      return sendResponse(res, {
        statusCode: 401,
...
chatRouter.get(
  ...
  async (req: JWTAuthRequest, res: any) => {
    try {
      if (!req.user?.userId) {
        return sendResponse(res, {
          statusCode: 401,
...
// Get online users
chatRouter.get("/online-users", async (req: JWTAuthRequest, res: any) => {
  try {
