...
(function (PermissionLevel) {
    ...
    PermissionLevel["TEACHER"] = "teacher";
...
const hasPermission = (userRole, resource, operation = "read") => {
    ...
    const hasOperationAccess = rule.allowedOperations.includes(operation) ||
...
const sanitizeData = (data, resource, userRole) => {
    ...
    if (rule.allowedRoles.includes(PermissionLevel.ADMIN) &&
...
const analyzeQueryWithSecurity = (query, userRole) => {
    const analysis = analyzeQuery(query);
    // Add security context
    analysis.securityContext = {
        requiresAuthentication: !analysis.category.includes("greeting") &&
...
const getDataAccessRequirements = (query, userRole) => {
...
const getSensitivityLevel = (query) => {
    const lowerQuery = query.toLowerCase();
    ...
    if (lowerQuery.includes("personal") ||
...
const filterResponseForSecurity = (response, userRole, sensitivityLevel) => {
...
const isTeacherOnly = (query) => teacherOnlyQueries.some((keyword) => query.includes(keyword));
const isStudentOnly = (query) => studentOnlyQueries.some((keyword) => query.includes(keyword));
const analyzeQuery = (query) => {
    ...
    // Technology topics
    if (lowerQuery.match(/\b(programming|coding|javascript|python|java|c\+\+|software|development)\b/)) {
        return {
            category: "programming",
            responses: generalResponses.technology.programming,
        };
    }
    if (lowerQuery.match(/\b(ai|artificial intelligence|machine learning|neural network|deep learning)\b/)) {
        return { category: "ai", responses: generalResponses.technology.ai };
    }
    if (lowerQuery.match(/\b(web|website|html|css|frontend|backend|full.?stack)\b/)) {
        return { category: "web", responses: generalResponses.technology.web };
    }
    // Science topics
    if (lowerQuery.match(/\b(physics|quantum|relativity|mechanics|electromagnetism)\b/)) {
    ...
    if (lowerQuery.match(/\b(chemistry|chemical|reaction|molecule|atom|bond)\b/)) {
        return {
            category: "chemistry",
    ...
    // Other topics
    if (lowerQuery.match(/\b(math|mathematics|algebra|geometry|calculus|statistics)\b/)) {
        return { category: "mathematics", responses: generalResponses.mathematics };
    }
    if (lowerQuery.match(/\b(history|historical|civilization|ancient|medieval|modern)\b/)) {
...
const buildStudentAnswer = (query, student) => {
...
const buildTeacherAnswer = (query, teacher) => {
    if (query.includes("profile") ||
        query.includes("who am i") ||
        query.includes("about me")) {
...
const buildGenericAnswer = (query) => {
...
export const assistantQuery = async (req, res) => {
    ...
    try {
        ...
        const hasAllPermissions = dataAccessRequirements.every((resource) => hasPermission(role, resource, "read"));
        if (!hasAllPermissions && dataAccessRequirements.length > 0) {
            ...
            return sendResponse(res, {
                statusCode: 403,
                success: false,
                message: "Access denied: Insufficient permissions for requested information",
                data: null,
            });
        }
